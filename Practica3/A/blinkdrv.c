			  ...
			  size_t len, loff_t *off)
{
	...
	for(i=0; i < NR_LEDS; i++){/*Por defecto todos leds apagados*/
	    color[i] = 0x0;
	}
	...
	tokens = strsep(&buf, ",");/*cortamos la entrada,separando por comas*/
	  if((n_args=sscanf(tokens, "%d:%x", &leds, &colorAux))==2){
		printk(KERN_INFO "led:%i-color:%i\n",leds,colorAux);
	  
	    if((leds >= 0 && leds <= 7)||(colorAux < 0x0 && colorAux > 0xFFFFFF)){
		color[leds] = colorAux;/*Guardar leds/colores de entrada*/
	    }
	    else{/*Argumento incorrecto*/
		retval= -EINVAL;
	   	goto out_error;
	    }
	 
	  }
	  else if(n_args>0){/*Si n_args es distinto de vacio*/
	    retval= -EINVAL;
	    goto out_error;
	  }
	...
	for(i=0; i< NR_LEDS; i++){/*Bucle para cada led*/
			   
	    message[0]='\x05';
	    message[1]=0x00;
	    message[2] = i;     
	    message[3]=((color[i]>>16) & 0xff);/*color correspondiente a cada led*/
	    message[4]=((color[i]>>8) & 0xff);
	    message[5]=(color[i] & 0xff);
		printk(KERN_INFO "color de i:%i\n",color[i]);
	    /*
